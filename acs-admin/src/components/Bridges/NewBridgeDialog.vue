  ...
  <Dialog :open="isOpen" @update:open="handleOpen">
    <DialogContent class="sm:max-w-[900px] overflow-y-auto">
      <DialogHeader>
        ...
        <DialogDescription>Configure a UNS bridge to forward messages between MQTT brokers</DialogDescription>
      </DialogHeader>
          ...
          <div class="rounded-lg border border-gray-300 p-5 bg-gray-100">
            ...
            <div class="grid grid-cols-2 gap-4 mt-4">
              <div class="flex flex-col gap-1">
                ...
                <Input
                    placeholder="Username for remote broker"
              ...
              <div class="flex flex-col gap-1">
                ...
                <Input
                    type="password"
                    placeholder="Password for remote broker"
        ...
        <DialogFooter :title="v$?.$silentErrors[0]?.$message">
          <Button v-if="bridgeType === 'outgoing'" :disabled="v$.$invalid || isSubmitting || bridgeType === 'incoming'" @click="formSubmit">
            <div class="flex items-center justify-center gap-2">
              <i :class="{'fa-solid': true, 'fa-plus': !isSubmitting, 'fa-circle-notch animate-spin': isSubmitting}"></i>
              <div>{{ isSubmitting ? 'Creating...' : 'Create Bridge' }}</div>
            </div>
          </Button>
        </DialogFooter>
      </DialogContent>
    ...
    methods: {
      ...
      async formSubmit() {
        ...
        try {
          ...
          const uuid = await this.s.client.ConfigDB.create_object(UUIDs.Class.Bridge)
