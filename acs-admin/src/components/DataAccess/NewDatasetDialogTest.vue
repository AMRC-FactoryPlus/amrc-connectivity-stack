...
export default {
  ...
  computed: {
    ...
    can_submit() {
      ...
      if (!this.dataset_components.every(c => is_valid_iso(c.from) || c.from == "")) return false
      if (!this.dataset_components.every(c => is_valid_iso(c.to) || c.to == "")) return false
      return true
    },
  },
  ...
  methods: {
    ...
    build_config() {
      ...
      if (this.structure_type === STRUCTURE_APPS.UNION) {
        return this.dataset_components.filter(c => is_valid_uuid(c.source))
      }
      return null
    },
    ...
    async submit() {
      ...
      for (const comp of this.dataset_components) {
        if (!this.da.structures.some(s => s.uuid === comp.source)) {
          this.error_message = `Component dataset ${comp.source} does not exist.`
          this.is_submitting = false
          return
        }
      }
