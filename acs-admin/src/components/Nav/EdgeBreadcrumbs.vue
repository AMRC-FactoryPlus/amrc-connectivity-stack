...
export default {
  ...
  computed: {
    ...
    // ╚═════════════════╝
    edgeClusters () {
      return useEdgeClusterStore().data.toSorted((a, b) => b.name < a.name)
    },
    currentEdgeCluster () {
    ...
    // ╚═════════╝
    nodes () {
      return useNodeStore().data.filter((node) => node.deployment?.cluster === this.$route.params.clusteruuid).toSorted((a, b) => b.name < a.name)
    },
    currentNode () {
    ...
    // ╚═══════════╝
    devices () {
      return useDeviceStore().data.filter((device) => device.deviceInformation?.node === this.$route.params.nodeuuid).toSorted((a, b) => b.name < a.name)
    },
    currentDevice () {
