...
function handleDelete() {
  useDialog({
    ...
    onConfirm: () => {
      s.client.Auth.delete_grant(props.row.original.uuid).then(async () => {
...
<template>
  <DropdownMenu>
    ...
    <DropdownMenuContent align="end" class="w-[250px]">
      <DropdownMenuGroup>
        <DropdownMenuItem @click="copy(props.row.original.principal.uuid)" class="cursor-pointer">
      ...
      <DropdownMenuGroup>
        <DropdownMenuItem @click="manage(props.row.original.principal)" class="cursor-pointer">
          <div class="flex items-center justify-center gap-2">
            <i class="fa-solid fa-fw fa-tag"></i>
            Manage Principal
          </div>
