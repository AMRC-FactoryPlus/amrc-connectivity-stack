...
keeps the relevant information in the Auth service.
...
and CCL permissions cannot be reasonable represented as a UUID.
...
* Functions which are UUIDs without a template definition represent base
  ...
  target of the permission, which should be a value.
...
strings. JSON arrays are represented without commas. For example, this
...
#### Sparkplug Node publishing
    ...
    ParticipateAsNode → [[]
        [let [addr [id [principal] sparkplug]]
            [map t [let [base [merge [addr] { type: [t] }]]
...
quickly become stale. A Special UUID, `Mine`, matches any object owned
by the requesting principal.
