...
class ConfigureDeviceConnectionAction
{
    ...
    public function execute(
        ...
        if ($device && $deviceConnection->node_id !== $device->node_id) {
            throw new ActionFailException('The connection could not be applied to this device because they do not belong to the same node.');
        }
        ...
        $connectionValidator->resolver()
                            ->registerRaw(
                                file_get_contents('https://raw.githubusercontent.com/AMRC-FactoryPlus/schemas/main/Device_Connection.json'),
        ...
        Storage::disk('device-connections')
                ->put($connectionDetailsFilename, json_encode($connectionConfig, JSON_THROW_ON_ERROR));
