...
class CreateNodeAction
{
    ...
    )
    {
        ...
        if (!in_array(config('app.env'), ['local', 'testing']) || env('K8S_DEPLOY_WHEN_LOCAL', false)) {
            ...
            $k8sSafeName = substr(
                str_replace('_', '-', (new BuildKerberosCRDNameAction())->execute($group, $nodeHostname, $nodeId)['data']),
                0,
                60
            );
            ...
            (new MakeConsumptionFrameworkRequest)->execute(
                ...
                payload: [
                    'uuid' => $nodeUuid,
                    'kerberos' => (new BuildKerberosPrincipalAction())->execute($group, $nodeHostname, $nodeId)['data'],
                ]
            );
