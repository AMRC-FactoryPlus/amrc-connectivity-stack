...
export default class Model extends EventEmitter {
    ...
    async config_put(q, json) {
        ...
        const rv = await this.db.txn({}, async query => {
            ...
            let ok = await query(`
                ...
                select $1, o.id, $3
                from object o
                where o.uuid = $2 on conflict (app, object) ${overwrite}
                    returning 1 ok
            `, [appid, q.object, json]);
    ...
    async config_delete(q) {
        ...
        let ok = await this.db.query(`
            delete
            from config c using object a, object o, app
            where a.id = app.id
              and a.uuid = $1
              and o.uuid = $2
              and c.app = a.id
    ...
    async dump_load(dump, overwrite) {
        ...
        for (const [klass, objs] of Object.entries(dump.objects ?? {})) {
            for (const obj of objs) {
                ...
                if (st > 299) {
                    debug.log("dump", "Dump failed [%s] on object %s (%s)", st, obj, klass);
                    return st;
                }
            }
        }
        for (const [app, objs] of Object.entries(dump.configs ?? {})) {
            for (const [object, conf] of Object.entries(objs)) {
                const st = await this.config_put(
                    ...
                    conf);
                if (st > 299 && st != 409) {
                    debug.log("dump", "Dump failed [%s] on config %s/%s", st, app, object);
                    return st;
                }
            }
        }
