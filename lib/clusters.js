...
export async function setup_clusters (ss, helm) {
    ...
    for (const ugrp of [group.flux.uuid, Clusters.Requirement.ServiceAccount]) {
        await ss.fplus.Auth.add_ace(ugrp, Git.Perm.Pull, group.shared.uuid);
    }

    for (const [key, repo] of Object.entries(ss.config.repoMirror)) {
        const uuid = config.repo[key].uuid;
        await mirror_repo({
            ss, uuid,
            source:     repo.url,
            ref:        repo.ref,
