...
export class Clusters {
    constructor (opts) {
        ...
        this.log        = this.fplus.debug.log.bind(this.fplus.debug, "edge");
    }
    ...
    async init () {
        ...
        /* Current global config entry */
        this.config = watcher.watch_config(Edge.App.Config, Edge.App.Config);
        /* [uuid, patch] requesting status updates */
        this.status_updates = new rx.Subject();

        this.clusters = this._init_clusters(cdb, watcher);
    ...
    _init_status_patches (cdb) {
        return this.status_updates.pipe(
            ...
            rx.concatMap(([uuid, patch]) =>
                ...
                rx.from(cdb.patch_config(Edge.App.Status, uuid, "merge", patch))
                /* Make sure we succeed before trying the next */
                    .pipe(rx.retry({ delay: 5000 }))),
        );
    }
