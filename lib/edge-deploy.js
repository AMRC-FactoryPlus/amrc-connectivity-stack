...
export class EdgeDeploy {
    ...
    async create_repo (name) {
        ...
        if (res.status != 200)
            throw `Git: can't create repo ${path}: ${res.status}`;
        return await res.json();
    }

    async create_cluster (req, res) {
        const { uuid, repo } = await this.create_cluster_objects(req.body);
        await this.populate_cluster_repo(repo, req.body.sources);
