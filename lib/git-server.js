...
export class GitServer {
    ...
    async init () { 
        ...
        app.all("/git/:group/:repo/:git(*)", this.wrap(this.by_path));
        app.all("/uuid/:uuid/:git(*)", this.wrap(this.by_uuid));
    ...
    async by_path (req, res, next) {
        ...
        req.url = `/uuid/${uuid}/${git}`;
        next();
    }

    async by_uuid (req, res, next) {
        const { uuid, git } = req.params;
        const push = git == "git-receive-pack" ||
        ...
        if (!await fs_p.stat(data)) {
            this.log("Creating repo %s", data);
            await git.init({
                fs,
                dir:            data, 
                bare:           true,
