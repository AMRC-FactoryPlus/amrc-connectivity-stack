...
export class GitServer {
    ...
    async del (req, res) {
        ...
        const ok = await this.check_perm(req.auth, info, Flux.Perm.Delete);
        if (!ok) return res.status(403).end();
