...
export class APIError extends Error {
    constructor (status) {
        super(http.STATUS_CODES[status] ?? `Error ${status}`);
        this.status = status;
    }
}
...
export class WebAPI {
    constructor (opts) {
        ...
        this.request_timeout = opts.request_timeout;
    ...
    async init () {
        ...
        /* Node's default requestTimeout (5 minutes) aborts the
         * connection if the whole request, including the body,
         * hasn't been received in time. This is too short for
         * services which accept large slow uploads. */
        if (this.request_timeout != null)
            this.http.requestTimeout = this.request_timeout;
