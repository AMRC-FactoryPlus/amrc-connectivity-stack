...
export class Auth extends ServiceInterface {
    ...
    decode_principal (req) {
        ...
        /* JWT-authenticated callers put fp_principal_uuid on req.auth
         * directly. A plain string is treated as a UUID identity if it
         * matches the UUID shape, otherwise as a Kerberos UPN (the
         * pre-JWT default). Existing callers that pass UPNs are
         * unaffected. */
        if (typeof req == "string")
            return [UUID_RX.test(req) ? "uuid" : "kerberos", req];
        
        if (typeof req != "object")
            throw new TypeError("Can't decode identity");
        const entries = Object.entries(req);
        if (entries.length != 1)
            throw new RangeError("Can't decode identity");
        return entries[0];
    }

    /* Create an ACL-checking function for a principal.
    ...
    async fetch_acl (princ_req, group) {
        ...
        if (type == null)
            throw new RangeError("Unrecognised principal request");
        ...
        const acl = await this.fetch_auth_acl(type, principal);
     ...
     * This function handles bootstrap ACLs where they exist but does
     * not handle wildcards or root permissions.
     *
     * @param type The identity type, `kerberos` or `uuid`.
     * @param principal The principal identity
     * @returns A list of {principal, permission, target} objects.
     */
    async fetch_auth_acl (type, principal) {
        if (type == "kerberos" && this.bootstrap_acl.has(principal))
            return this.bootstrap_acl.get(principal);

        const url = type == "uuid" ? urljoin("v2", "acl", principal)
            : urljoin("v2", "acl", type, principal);
        const [st, json] = await this.fetch_raw_acl(url);
        if (st != 200)
            this.throw(`Failed to read ACL for ${principal}`, st);
        return json;
    }
    ...
    async _resolve_by_addr (address) {
        ...
        if (addr.isDevice())
            throw new RangeError(`${addr} is a Device address`);
