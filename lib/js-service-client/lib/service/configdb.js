...
export class ConfigDB extends ServiceInterface {
    ...
    async get_config_with_etag (app, obj) {
        const [st, json, etag] = await this.fetch(`v2/app/${app}/object/${obj}`);
        if (st == 404) return [];
        if (st != 200)
    ...
    async list_configs (app) {
        const [st, json] = await this.fetch(`v2/app/${app}/object`);
        if (st == 404) return;
        if (st != 200)
     ...
     *
     * @deprecated The MetaDB will not implement this.
     * @arg app The application UUID
    ...
    /** List objects by config entry.
     *
     * Finds all objects with a config entry exactly matching the one
     * supplied. Results may be restricted by class.
     *
     * @arg app The App to search
     * @arg config The config entry to match against
     * @arg klass (optional) Restrict results to this class
     * @returns An array of UUIDs
     */
    async find (app, config, klass) {
        const kpath = klass ? `/class/${klass}` : "";

        const [st, uuids] = await this.fetch({
            method:     "POST",
            url:        `v2/app/${app}${kpath}/find`,
            body:       config,
        });

        if (st != 200) {
            const kmsg = klass ? ` in ${klass}` : "";
            this.throw(`Find for ${app}${kmsg} failed`, st);
        }
        return uuids;
    }

    /** Find a single object by config entry.
     *
     * Calls `find` and expects the result to contain no more than one
     * object. Throws if we get multiple results.
     *
     * @arg app The App to search
     * @arg config The config entry to match against
     * @arg klass (optional) Restrict results to this class
     * @returns A single UUID, or undefined
     */
    async find_unique (app, config, klass) {
        const uuids = await this.find(app, config, klass);
        if (uuids.length > 1) {
            const kmsg = klass ? ` in ${klass}` : "";
            this.throw(`Find for ${app}${kmsg} returned multiple results`);
        }
        return uuids[0];
    }
    ...
    /** Find a single object by config entry.
     *
     * This is a compatibility method that calls `find_unique`.
     *
     * This used to call `search` but that endpoint is deprecated and
     * not implemented by the MetaDB. The `query` parameter will be used
     * directly as the config entry to match. This is only compatible
     * with `search` as long as only top-level properties are used; the
     * JSONpath feature of `search` is no longer supported. Properties
     * explicitly mapped to `undefined` will not be included in the
     * request; this matches the `search` behaviour of requiring these
     * to not be present. Properties omitted from the query must also be
     * not present in the config; this is a change from the `search`
     * behaviour, which would ignore additional properties.
     *
     * Arguments are in an object.
     *
     * @deprecated Use find_unique for new code
     * @arg app The App to search
     * @arg query The config entry to search for
     * @arg klass Restrict search to objects of this class
     * @returns The UUID of the single object which matches the query.
     */
    resolve (opts) {
        return this.find_unique(opts.app, opts.query, opts.klass);
    }
