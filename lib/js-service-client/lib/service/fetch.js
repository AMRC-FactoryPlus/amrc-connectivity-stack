...
import { jwt_context } from "../jwt-context.js";
...
export class Fetch extends ServiceInterface {
    ...
    constructor (fplus) {
        ...
        /** The default cache strategy for this Fetch object.
         * This is the fallback default; it is overridden by an explicit
         * `cache` option in the `fetch` call and also by a `cache`
         * option in the ServiceClient.
         *
         * @deprecated Use `uncached()` on the ServiceClient instead.
         */
        this.cache = "default";
    }
...
*/
    async fetch (opts) {
        try {
            ...
            /* ServiceClient default overrides our local default */
            opts.cache ??= this.fplus.opts.cache ?? this.cache;
    ...
    async do_fetch (url, opts) {
        /* If the inbound request authenticated with a Keycloak JWT,
         * forward it verbatim. The downstream service validates it
         * against the same Keycloak realm (via service-api's auth
         * middleware) and runs its ACL checks against the JWT's
         * principal. No exchange, no per-service token caching, no
         * extra round-trip.
         *
         * On 401 we surface the error rather than retrying: a JWT
         * either validates or it doesn't, and the only way a retry
         * could help is for transient JWKS issues which jose handles
         * internally. */
        const jwt_ctx = jwt_context.getStore();
        if (jwt_ctx?.jwt) {
            const tok = jwt_ctx.jwt.slice(0, 8);
            this.debug.log("token", `Forwarding JWT ${tok}... to ${url}`);
            return await this._real_fetch(url,
