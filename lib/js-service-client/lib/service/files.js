...
export class Files extends ServiceInterface {
    ...
    async save_file(uuid, outputPath){
        if(this.fplus.opts.browser){
            throw new Error("Method not supported in browser");
        }
    ...
    async get_file_stream(uuid){
        const [st, body] = await this.fetch({
            ...
            /* Bodies are streamed straight to disk by callers and can be
             * large; the HTTP cache would buffer the whole response into
             * memory (and keep it there) just to service a future request
     ...
     * @param opts Optional extras. `opts.headers` are merged into the
     *   request headers, e.g. to pass a `device-uuid` header.
     */
    async upload_file_with_uuid(uuid, file, opts = {}){
        let fileBody;
        let fileName;
