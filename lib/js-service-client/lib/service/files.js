...
export class Files extends ServiceInterface {
     ...
     */
    async upload_file_with_uuid(uuid, filePath){
        const isNode = typeof window === 'undefined' && typeof process !== 'undefined';
        if(!isNode){
            this.throw("Method not supported in browser");
        }
    ...
    /**
     * Uploads a file to the file service from the browser.
     * @platform Browser only - Not supported in Node.js
     * @param file The file to upload.
     * @param uuid UUID of the file object.
     */
    async upload_file_with_uuid_from_browser(file, uuid){
        const isNode = typeof window === 'undefined' && typeof process !== 'undefined';
        if(isNode){
     ...
     */
    async upload_file_from_browser(file){
        const isNode = typeof window === 'undefined' && typeof process !== 'undefined';
        if(isNode){
            this.throw("Method not supported in node");
        }
        ...
        await this.upload_file_with_uuid_from_browser(file, uuid);
    ...
    async upload_file(filePath){
        ...
        await this.upload_file_with_uuid(uuid, filePath);
        const fileName = path.basename(filePath);
        const extension = fileName.split(".")[1];
        const fileTypeClass = this.file_types.get(extension.toLowerCase());
        if(!fileTypeClass)return;
