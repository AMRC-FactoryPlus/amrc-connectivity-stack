...
export class Monitor {
    ...
    run () {
        this.nodes.subscribe(msg => this.log("SP nodes: %s", msg));
    ...
    _init_nodes () {
        ...
        return rxx.k8s_watch({
            ...
            rx.map(ns => {
                ...
                return uuids.map(n => [n.uuid, n.edgeAgent])
                    .fromEntrySeq().toMap();
            }),
            nodes => {
                /* Work out what's changed. We couldn't just have used the
                 * k8s change notifications directly because the F+ UUID in
                 * a k8s SPNode can be changed. */
                const changes = nodes.pipe(
                    rx.map(ns => ns.keySeq().toSet()),
                    ...
                    rx.tap(ch => this.log("CHANGE: %o", ch)),
                    rx.share(),
                );
                const [start, stop] = rx.partition(changes, ch => ch[0]);
                /* When we get a "start", start a new sequence for that
                 * UUID. Stop the sequence when we get a "stop". */
                return start.pipe(
                    rx.tap(st => this.log("START: %o", st)),
                    rx.flatMap(([, uuid]) => rx.interval(5000).pipe(
                        rx.map(n => `${uuid}: ${n}`),
                        rx.takeUntil(stop.pipe(
                            rx.tap(st => this.log("STOP: %o", st)),
                            rx.filter(ch => ch[1] == uuid),
                            rx.tap(st => this.log("STOP for %s: %o", uuid, st)),
                        )),
                    )),
                );
            },
