...
export class WatchFilter extends PathFilter {
    ...
    handle (session, sub) {
        ...
        const meth = (head ? null : request.method) ?? "GET";
        if (meth != this.method) return;

        const seq = this.check_path(sub.request.url);
        return head ? seq : seq?.pipe(rx.map(update => {
...
export class SearchFilter extends PathFilter {
    handle (session, sub) {
        if (sub.method != "SEARCH") return;
        return this.check_path(sub.parent);
    }
}
    ...
    validate_subscription (sub) {
        switch (sub.method) {
            ...
            case "SEARCH":
                ...
                if (!sub.path.endsWith("/")) return false;
                return true;
        }
        return false;
    }
