...
//        return dbr.rows;
    ...
    async record_alert (opts) {
        ...
        if (find.rowCount > 0) {
            ...
            return await this.create_alert(opts);
        }
    }
    ...
    async update_alert_info (id, opts) {
        await this.query(`
            ...
            where id = $2
        `, [opts.id, opts.dev_id, opts.type_id, opts.metric, opts.active]);
    }

    update_alert_active (id, active) {
        await this.query(`
            update alert
            set active = $2
            where id = $1
