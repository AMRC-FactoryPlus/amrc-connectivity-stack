...
export class Reconciler {
    ...
    nodes_observable () {
        ...
        return new rx.Observable(obs => {
            ...
            rx.tap(v => this.log("Node update: %o", v)),
            rx.scan(
                (nodes, [act, node]) => act == "DELETED"
                    ? nodes.delete(node.uuid)
                    : nodes.set(node.uuid, node),
                new Map()),
            rx.distinctUntilChanged(deep_equal.deepEqual),
            rx.debounceTime(100),
            rx.tap(v => this.log("Node state debounced: %o", v)),
        );
    }

    run () {
        this.log("Subscribing nodes seq");
        this.nodes.subscribe(ns => this.log("NODES: %o", ns));
    }
}
