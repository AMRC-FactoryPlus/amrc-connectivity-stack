...
export class SparkplugNode {
    ...
    _init_metrics () {
        const build_metrics = status => imm.Seq().concat(
            ...
            status.entrySeq()
                .map(([uuid, brs]) => [`Repositories/${uuid}`, brs])
                .flatMap(([prefix, branches]) => imm.Seq().concat(
                    Metric.of(`${prefix}/Exists`, "Boolean", true),
    ...
    async nbirth () {
        ...
        this.client.publishNodeBirth({ metrics: metrics.toArray() });
    }
