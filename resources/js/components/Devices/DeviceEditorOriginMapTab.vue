...
<template>
  <div
      class="flex flex-col flex-grow flex h-page overflow-y-auto">
    <div
        v-if="loadingExistingConfig"
    ...
    <div
      ...
      <div
        ...
        <div
          ...
          <div
            ...
            <button
                type="button"
                @click.stop="schemaBrowserVisible=true"
      ...
      <schema-browser-overlay
          :show="schemaBrowserVisible"
          @close="schemaBrowserVisible=false"
    ...
    <div
      ...
      <div
        ...
        <h2 v-if="schema"
            class="font-bold text-brand p-3">
          {{
            schema.title
          }}</h2>
        <button
            @click="showCDSImport = true"
            class="fpl-button-brand h-10 mx-1">
          Import
          CDS
      ...
      <div
          class="flex flex-grow overflow-auto">
        <div
            class="flex flex-col overflow-y-auto flex-shrink-0 w-1/4">
          <SchemaGroup
              :key="rerenderTrigger"
          ...
          <div
            ...
            <!--<button @click="save(false)" class="fpl-button-secondary h-10">-->
            <!--  Save Draft-->
            ...
            <button
                v-if="isDirty"
                @click="save(true)"
                class="fpl-button-error h-10 ml-2"
          ...
          <div
              v-else
              class="flex items-center justify-center p-3 mt-auto transition-all">
            <button
                disabled
                class="fpl-button-brand h-10 ml-2 w-full cursor-not-allowed opacity-50">
              Validating
        ...
        <div
            v-if="selectedMetric"
            class="flex flex-col h-full overflow-y-auto border-l">
          <SparkplugMetric
              :selected-metric="selectedMetric"
              :model="model"
  ...
  methods: {
    ...
    applyMapping(mapping) {
      ...
      Object.keys(mapping).forEach(e => {
        this.set(mapping[e].schemaMapping.namePath.join('/'), this.renameKeys(mapping[e].metric), this.model, '/');
      });
      this.markDirty('Mapping applied');
    },
    ...
    renameKeys(e) {
      return {
        ...
        Deadband: e.deadBand ?? null,
        Documentation: e.docs ?? null,
        Eng_High: e.engHigh ?? null,
        Eng_Low: e.engLow ?? null,
        Eng_Unit: e.engUnit ?? null,
        Method: e.method ?? null,
        Path: e.path ?? null,
        Record_To_Historian: e.recordToDB ?? null,
        Sparkplug_Type: this.remapTypes(e.type) ?? null,
        Value: e.value ?? null,
      };
    },
