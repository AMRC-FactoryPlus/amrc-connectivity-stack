...
<template>
  <div v-if="schema && schemaLoading === false" class="flex flex-col p-4">
    <div class="grid grid-cols-2 gap-10 mb-6">
      <div class="flex flex-col">
        ...
        <input class="font-light text-sm text-gray-400 w-full focus:outline-none focus:bg-gray-50 focus:p-1 mr-3 group" v-model="localModel.Documentation"></input>
      </div>
...
export default {
  ...
  mounted () {
    ...
    axios.post('/api/github-proxy/', {
      ...
      this.localModel.Method = data.data.properties.Method.default;
  ...
  methods: {
    populateForm () {
      ...
      Object.keys(this.selectedMetric.metric.properties).forEach(e => {
        // If we have one in the master model then use that
        if (existing && e in existing) {
          this.$set(this.localModel, e, existing[e]);
        }
        // Otherwise init the default value
