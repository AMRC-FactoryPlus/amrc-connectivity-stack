...
export default class MQTTClient {
    ...
    async run() {
        ...
        mqtt.on("connect", this.on_connect.bind(this));
    ...
    private writeMetrics(payload, topic: Topic) {
        payload.metrics.forEach((metric) => {
            ...
            this.republish(birth, topic, metric.value, metric.timestamp)

        });
    }

    republish(birth, topic: Topic, value, timestamp) {
        ...
        switch (birth.type) {
            ...
            case "Int64":
                ...
                writeApi.writePoint(new Point(`${metricName}:i`).intField('value', numVal).timestamp(timestamp));
                break;
            case "UInt8":
            case "UInt16":
            case "UInt32":
